# DEPRECATED ----- NEEDS TO BE UPDATED TO REFLECT NEW BACKEND.
# REASON: ABUSED TABLE SCAN.
...
def lambda_handler(event, context):
    ...
    response = table.scan(
        FilterExpression=Attr("VersusQueue").size().lt(
            10) and Attr("VersusViews").size().gt(1)
    )

    for user in response['Items']:
        user_id = user['UserID']
        restaurant_combinations = set(
            combinations(user['VersusViews'].keys(), 2))
        versus_pairs = set([tuple(l) for l in user['VersusPairs']])  # O(n^2)
        restaurant_combinations = (
            restaurant_combinations | versus_pairs) - (restaurant_combinations & versus_pairs)
